...
// Files indicator files handler
type Files struct {
	group          mir.Group  `mir:"admin"`
	getFiles       mir.Get    `mir:"/files/{name}"`
	uploadFile     mir.Post   `mir:"/files/{name}"`
	updateFilename mir.Put    `mir:"/files/{name}/{id}/{filename}"`
	deleteFiles    mir.Delete `mir:"/files/{name}/{ids}"`
}

// GetFiles : Return array of files for site
func (f *Files) GetFiles(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
...
func (f *Files) UpdateFilename(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
	id := vars["id"]
...
func (f *Files) UploadFile(w http.ResponseWriter, r *http.Request) {
...
func (f *Files) DeleteFiles(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
	fileIds := vars["ids"]
