...
// Users indicator users handler
type Users struct {
	group              mir.Group  `mir:"admin"`
	createUser         mir.Post   `mir:"/users"`
	authenticateUser   mir.Post   `mir:"/users/authenticate"`
	verifyUser         mir.Post   `mir:"/users/verify/{id}"`
	deleteUser         mir.Delete `mir:"/user"`
	updateUserPassword mir.Put    `mir:"/user/password"`
	updateUser         mir.Put    `mir:"/profile"`
	chargeAccount      mir.Post   `mir:"/account/charge"`
	removeSubscription mir.Delete `mir:"/account/subscription"`
	getSelfUser        mir.Get    `mir:"/user"`
}
...
func (u *Users) GetSelfUser(w http.ResponseWriter, r *http.Request) {
...
func (u *Users) RemoveSubscription(w http.ResponseWriter, r *http.Request) {
	id := utils.GetUserObjectID(r)
	user, err := database.GetUserByID(id)
...
func (u *Users) ChargeAccount(w http.ResponseWriter, r *http.Request) {
...
func (u *Users) UpdateUserPassword(w http.ResponseWriter, r *http.Request) {
	id := utils.GetUserObjectID(r)
