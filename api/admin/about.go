...
// About indicator about handler
type About struct {
	Chain       mir.Chain `mir:"-"`
	group       mir.Group `mir:"admin"`
	getAbout    mir.Get   `mir:"/about/{name}"`
	updateAbout mir.Put   `mir:"/about/{name}"`
}

// GetAbout : return about content of site
func (a *About) GetAbout(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
...
func (a *About) UpdateAbout(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
