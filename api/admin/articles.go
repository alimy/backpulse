...
// Articles indicator articles handler
type Articles struct {
	group         mir.Group  `mir:"admin"`
	getArticles   mir.Get    `mir:"/articles/{name}"`
	getArticle    mir.Get    `mir:"/articles/{name}/{id}"`
	updateArticle mir.Put    `mir:"/articles/{name}"`
	deleteArticle mir.Delete `mir:"/articles/{name}/{id}"`
}
...
func (a *Articles) GetArticle(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
	id := vars["id"]
...
func (a *Articles) UpdateArticle(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
...
func (a *Articles) DeleteArticle(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
	id := vars["id"]
