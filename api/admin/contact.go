...
// Contact indicator contact handler
type Contact struct {
	group         mir.Group `mir:"admin"`
	getContact    mir.Get   `mir:"/contact/{name}"`
	updateContact mir.Put   `mir:"/contact/{name}"`
}

// GetContact : return contact data about site
func (c *Contact) GetContact(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
...
func (c *Contact) UpdateContact(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	name := vars["name"]
