...
// Photos indicator photos handler
type Photos struct {
	group               mir.Group  `mir:"admin"`
	uploadPhoto         mir.Post   `mir:"/photos/{name}"`
	deletePhotos        mir.Delete `mir:"/photos/{ids}"`
	updatePhotosIndexes mir.Put    `mir:"/photos/{name}/{id}/indexes"`
}

// UpdatePhotosIndexes : change order of photos
func (p *Photos) UpdatePhotosIndexes(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	siteName := vars["name"]
	galleryID := vars["id"]
...
func (p *Photos) DeletePhotos(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	ids := vars["ids"]
...
// UploadPhoto handler
func (p *Photos) UploadPhoto(w http.ResponseWriter, r *http.Request) {
